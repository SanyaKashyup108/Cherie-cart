...
const Cart = ({ location, getLocation }) => {
  const { cartItem, setCartItem, updateQuantity, deleteItem, sharedCartId, setSharedCartId } = useCart(); // ✅ Inside function
  const { user } = useUser();
...
onClick={async () => {
  ...
  setSharedCartId(cartId); // ✅ Set it globally for real-time sync
  alert("Cart link copied! Share it with your friend ✨");
}}
  className="bg-blue-500 text-white px-4 py-2 rounded-md my-4 hover:bg-blue-600 transition-all"
>
  Share Cart 🛒
</button>
